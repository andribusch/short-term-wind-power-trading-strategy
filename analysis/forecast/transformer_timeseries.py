...
class Generator(nn.Module):
	...
	At the end of the encoder on the picture.
	"""
...
def data_gen(V, batch, nbatches):
	...
	row_shape = X0.shape[0]
	for i in range(nbatches):
		data1 = torch.from_numpy(X0.reshape(row_shape, 8))  # .long()
		data1[:, 0] = 1
		data2 = torch.from_numpy(Y0.reshape(row_shape, 8))  # .long()
		data2[:, 0] = 1
...
class SimpleLossCompute:
	...
	def __call__(self, x, y, norm):
		x = self.generator(x)
		rows_reshape = x.shape[0]
		x = torch.sum(x.reshape(rows_reshape, 7, -1), (2))
...
def setup_model(look_back, dataset, V):
	"""
	Setup all the model and data
	This function is self made
	"""
	...
	return X0, Y0, model, model_opt, criterion


def train(epochs, model, criterion, model_opt, V):
...
if __name__ == '__main__':
	# Get data
	df = pd.read_excel('../../data/imbalance_strategy/full_case_study/imbalance_volumes_2020_2022.xlsx')
	...
	# Look-back is the vector size, that we take X_t-6 to X_t (8 floats)
	look_back = 8
	np.random.seed(7)
	...
	X0, Y0, model, model_opt, criterion = setup_model(look_back, dataset, V)

	# Important: Train model or if not, then use transfer learning

	model = train(EPOCHS, model, criterion, model_opt, V)

	# Important: Do not train model, use pre-trained!
	...
	# Important --------- Validation and visualization of results -------------------
	place = 1600
