...
class BasicValuation(ValuationAlgorithm):
    """
    ...
    def valuate(self):
        ...
        MIN_VOLUME = 5
        MIN_VOLUME_PROBALBILITY = 0.5
        MIN_PREMIUM = 7
        MIN_PREMIUM_PROBABILITY = 0.3
    ...
    def forecast_volume(self):
        activations_volumes = []
        for hour in range(24):
            filtered_df = self.activations_data[
                self.activations_data.apply(lambda x: x['Datetime'].hour == hour, axis=1) != 0]
            filtered_df = filtered_df[filtered_df.apply(lambda x: x['market_regulation_total'] != 0, axis=1) != 0]
            try:
    ...
    def forecast_volume_probability(self) -> list:
        ...
        for hour in range(24):
            # Take this hour
            filtered_df_hour = self.activations_data[
                self.activations_data.apply(lambda x: x['Datetime'].hour == hour, axis=1) != 0]
            # Take only, where activated
            filtered_df = filtered_df_hour[
                filtered_df_hour.apply(lambda x: x['market_regulation_total'] != 0, axis=1) != 0]
            soodsad = len(filtered_df.index)
            koik = len(filtered_df_hour)
    ...
    def forecast_price_premium(self, tomorrow_date: datetime.date):
        ...
        for hour in range(24):
            # Group by hours
            # filtered_mfrr_df = self.mFRR_prices[
            #     self.mFRR_prices.apply(lambda x: x['Datetime'].hour == hour, axis=1) != 0]
            # filtered_spot_df = self.spot_prices[
            #     self.spot_prices.apply(lambda x: x['Datetime'].hour == hour, axis=1) != 0]
            ...
            # filtered_df = filtered_df[filtered_df.apply(lambda x: x[''] != 0, axis=1) != 0]
            filtered_df = filtered_all_data_hour[
                filtered_all_data_hour.apply(lambda x: (x['mFRR_UP_balancing_price'] - x['SPOTEE']) > 0,
                                             axis=1) != 0]
            try:
    ...
    def forecast_premium_probability(self) -> list:
        ...
        for hour in range(24):
            ...
            filtered_all_data_hour = input_data[
                input_data.apply(lambda x: x['Datetime'].hour == hour, axis=1) != 0]
            koik = len(filtered_all_data_hour.index)

            # Soodsad: Premium is same or bigger
            filtered_df = filtered_all_data_hour[
                filtered_all_data_hour.apply(lambda x: (x['mFRR_UP_balancing_price'] - x['SPOTEE']) >= forecasted_value,
                                             axis=1) != 0]
            soodsad = len(filtered_df.index)
...
if __name__ == '__main__':
    ...
    values_for_tomorrow.to_excel('mFRR_outputs_' + str(datetime.today().date() + timedelta(days=1)) + '.xlsx')
